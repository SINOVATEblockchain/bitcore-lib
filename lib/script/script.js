 ...
 * @returns {boolean} true if this is a valid standard OP_RETURN output
 */
Script.prototype.isBurnDataOut = function() {
  return this.chunks.length >= 2 &&
    ...
    (this.chunks.length === 2 ||
      (this.chunks.length === 3 &&
        this.chunks[2].buf &&
        this.chunks[2].buf.length <= Script.OP_RETURN_STANDARD_SIZE &&
        this.chunks[2].length === this.chunks.len));
};

/**
 * Retrieve the associated information for this script.
 * In the case of a standard TX_BURN_DATA, return the data
 * @returns {Buffer}
 */
Script.prototype.getBurnDataInfo = function() {
  if (this.isBurnDataOut()) {
    if (_.isUndefined(this.chunks[2])) {
      return Buffer.alloc(0);
    } else {
      return Buffer.from(this.chunks[2].buf);
    }
...
/**
 * Retrieve the associated destination for this script.
 * In the case of a standard TX_BURN_DATA, return the data
 * @returns {Buffer}
 */
Script.prototype.getBurnDataDestination = function() {
  if (this.isBurnDataOut()) {
...
Script.types.BURN_DATA = 'Data push';
...
Script.outputIdentifiers.BURN_DATA = Script.prototype.isBurnDataOut;
 ...
 * @param {(string)} encoding - the type of encoding of the string
 */
Script.buildBurnDataOut = function(to, data, encoding) {
  $.checkArgument(!_.isUndefined(to));
  $.checkArgument(to instanceof PublicKey || to instanceof Address || _.isString(to));
  if (to instanceof PublicKey) {
    to = to.toAddress();
